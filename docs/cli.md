...
| Create replay job | `replay:write` | `go run ./cmd/whcp replay-jobs create --event-id evt_... --config-mode original --rate-limit-per-minute 60 --require-approval --reason-code support_investigation --reason "customer replay request" --api-key "$WEBHOOKERY_API_KEY"` | Replay is scheduled or awaits approval with structured reason evidence. | Duplicate side effects |
