...
func (s *Server) Routes() http.Handler {
	...
	r.Route("/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/incidents/{incident_id}", s.getIncident)
			r.Post("/incidents/{incident_id}/events", s.addIncidentEvent)
			r.Delete("/incidents/{incident_id}/events/{event_id}", s.removeIncidentEvent)
			r.Post("/incidents/{incident_id}/generate-report", s.generateIncidentReport)
			r.Get("/incidents/{incident_id}/report", s.getIncidentReport)
			r.Post("/incidents/{incident_id}/evidence-export", s.createIncidentEvidenceExport)
			...
			r.Get("/replay-approval-policies", s.listReplayApprovalPolicies)
			r.Post("/replay-approval-policies", s.createReplayApprovalPolicy)
			r.Delete("/replay-approval-policies/{policy_id}", s.disableReplayApprovalPolicy)
